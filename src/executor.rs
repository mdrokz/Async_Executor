use std::{fmt::Debug, future::Future, pin::Pin, sync::mpsc::{channel, Receiver, Sender}, sync::{Arc, Mutex, mpsc::{SyncSender, sync_channel}}, task::{Context, Poll, Wake}};
...
pub struct SyncExecutor<'a, T> {
    ready_queue: Receiver<Arc<SyncTask<'a, T>>>,
}
...
#[derive(Clone)]
pub struct SyncSpawner<'a, T> {
    task_sender: SyncSender<Arc<SyncTask<'a, T>>>,
}
...
impl<'a, T> Spawner<'a, T> {
    pub fn spawn(&self, future: impl Future<Output = T> + 'a + Send) {
...
impl<'a, T> SyncSpawner<'a, T> {
    pub fn spawn(&self, future: impl Future<Output = T> + 'a + Send) {
        let future = Box::pin(future);

        let task = Arc::new(SyncTask {
            future: Mutex::new(Some(future)),
            task_sender: self.task_sender.clone(),
        });
...
impl<'a, T> Wake for Task<'a, T> {
    ...
    fn wake_by_ref(self: &Arc<Self>) {
        ...
        self.task_sender.send(cloned).unwrap();
    }
}

impl<'a, T> Wake for SyncTask<'a, T> {
    fn wake(self: Arc<Self>) {}
...
impl<'a, T> SyncExecutor<'a, T> {
    pub fn run_queued(&self, callback: Box<dyn Fn(T) + 'a>) {
...
pub fn new_queue_executor_and_spawn<'a, T>(bound: usize) -> (SyncExecutor<'a, T>, SyncSpawner<'a, T>) {
    let (task_sender, ready_queue) = sync_channel(bound);

    (SyncExecutor { ready_queue }, SyncSpawner { task_sender })
}
